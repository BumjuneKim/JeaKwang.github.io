							...
							<ul>
								...
								<li><strong>Status-Line : 응답 메시지의 첫번째줄</strong>
									<ul>
										...
										<li>1xx : 정보성 / 2xx : 성공 / 3xx : 리다이렉트 / 4xx : 클라이언트 오류 / 5xx : 서버 오류</li>
										</li>
									</ul>
								</li>
							</ul>
...
{% endhighlight %}
						...
						<ul>
							...
							<li>Expires : 응답에의 바디가 캐시로서의 유효기간을 나타냅니다.(절대시간)</li>
...
{% endhighlight %}
			...
			<section id="session">
				...
				<section data-transition="fade">
					...
					<p style="color: white;">
						HTTP는 '스테이트리스'한 특성을 가지고 있어서 클라이언트의 정보를 저장하지는 않지만, <br> 때로는 그 정보를 기억해야할 경우에 Session 과 Cookie를 사용할 수 있다.
					</p>
					<table>
						<thead>
							<tr>
						...
						<tbody>
								<tr>
									<td>서버가 클라이언트측에 부여한 ID를 세션</td>
									<td>서버측에서 클라이언트 측에 상태 정보를 저장하고 추출할 수 있는 메커니즘</td>
								</tr>
								<tr>
									<td>클라이언트와 서버간에 통신연결이 활성화되 되었다는 의미를 지님</td>
									<td>클라이언트의 매 요청마다 웹 브라우저로부터 서버에게 전송되는 정보패킷</td>
								</tr>
								<tr>
									<td>서버의 정보를 저장</td>
									<td>클라이언트의 정보를 저장</td>
			...
			<section id="https">
				...
				<section data-transition="fade">
					...
					<div class="fragment fade-in">
						<div class="box"></div>
						<div class="boxDesc">
							Session Layer에 있는 SSL/TLS 프로토콜을 활용하여 통신로를 암호화하고<br> 클라이언트와 서버 간에 주고받는 데이터를 보호하여,<br> 도청을 방지할 수 있습니다.
						</div>
					</div>
				</section>
				...
				<section data-transition="fade">
					...
					<p class="fragment fade-in">
						1. 공개키가 유출되어도 비공개키가 없으면 복호화 불가능 <br>
						2. 데이터를 제공한 사람의 신원 보장
					</p>
				</section>
  ...
  Reveal.addEventListener( 'fragmentshown', function( event ) {
  	// var target = event.fragment;
  	// if($(target).hasClass("fade-out")){
  	// 	$(target).hide();
  	// }
